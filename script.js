...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const chartObs = new IntersectionObserver(entries => {
    entries.forEach(e => {
      if (e.isIntersecting) {
        const bars = [...e.target.querySelectorAll('.chart-bar-fill')];
        const maxVal = Math.max(...bars.map(b => parseFloat(b.dataset.value)));
        bars.forEach((bar, i) => {
          const pct = (parseFloat(bar.dataset.value) / maxVal) * 100;
          setTimeout(() => {
            bar.style.width      = pct + '%';
  ...
  if (chartWrap) chartObs.observe(chartWrap);
